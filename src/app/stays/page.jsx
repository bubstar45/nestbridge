...
export default function StaysPage() {
  ...
  const listings = useMemo(
    () => rawListings.filter(l => categoryDef.match(l)),
    [rawListings, categoryDef]
  )

  // Count per category so we can show badges and grey out empty ones
  ...
  const handleSearch = () => {
    setActive(null)
    setCategory('All') // reset category on new search so results are unfiltered first
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Search bar */}
      <div className="border-b bg-white sticky top-16 z-40 py-5 px-6">
          ...
          <div className={`flex items-stretch bg-white border rounded-full transition-all ${active ? 'shadow-2xl' : 'shadow-md hover:shadow-lg'}`}>
            ...
            <div className="flex items-center pr-2 pl-2">
              <button
                ...
                className="flex items-center gap-2 bg-stay-500 text-white px-5 py-3 rounded-full font-semibold text-sm hover:bg-stay-600 transition-colors">
          ...
          {active === 'where' && (
            <div className="absolute top-full left-0 mt-3 bg-white border rounded-3xl shadow-2xl p-5 w-80 z-50 max-h-80 overflow-y-auto">
              <p className="text-xs font-bold text-gray-400 mb-3 uppercase tracking-widest">
                {city.length > 0 ? 'Matching destinations' : 'Available destinations'}
              </p>
              ...
              {filteredSuggestions.map(s => (
                <button
                  key={s.city}
                  onClick={() => { setCity(s.city); setActive('when') }}
                  className="flex items-center gap-3 w-full px-3 py-2.5 hover:bg-gray-50 rounded-2xl text-left transition-colors">
                  <div className="w-9 h-9 bg-gray-100 rounded-xl flex items-center justify-center text-base shrink-0">📍</div>
                  <div>
                    <p className="text-sm font-medium text-gray-900">{s.city}</p>
                    <p className="text-xs text-gray-400 capitalize">{s.type} available</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 bg-white border rounded-3xl shadow-2xl p-6 z-50 w-[580px]">
              <div className="flex items-start gap-8">
                <button
                  ...
                  className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-600 mt-1 shrink-0">
                  ‹
                </button>
              ...
              {checkIn && (
                <div className="flex justify-end mt-4">
                  <button onClick={() => { setCheckIn(''); setCheckOut('') }} className="text-sm underline text-gray-500 hover:text-gray-900">
                    Clear dates
                  </button>
                </div>
              )}
      ...
      <div className="border-b px-6 py-3 bg-white">
        <div className="max-w-7xl mx-auto flex gap-6 overflow-x-auto pb-1">
          {CATEGORIES.map(c => {
            ...
            return (
              <button
                ...
                disabled={isEmpty}
                className={`
                  flex flex-col items-center gap-1 shrink-0 pb-2 border-b-2 transition-colors relative
                  ${active
                ...
                `}>
                <span className={`text-xl transition-all ${active ? '' : isEmpty ? 'grayscale opacity-30' : ''}`}>
                  {c.icon}
                </span>
                <span className="text-xs font-medium whitespace-nowrap">{c.label}</span>
                {/* Count badge — only show when not active and not loading */}
                {!isLoading && !active && !isEmpty && (
                  <span className="absolute -top-1 -right-2 bg-gray-100 text-gray-500 text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">
                    {count}
                  </span>
                )}
              </button>
        ...
        <p className="text-sm text-gray-500">
          {isLoading
            ...
            : category === 'All'
              ...
              : `${listings.length} ${category.toLowerCase()} stay${listings.length !== 1 ? 's' : ''}`
          }
          {nights > 0 && <span className="ml-1 text-gray-400">· {nights} night{nights !== 1 ? 's' : ''}</span>}
        </p>
        <div className="flex items-center gap-3">
          {/* Active category pill with clear button */}
          {category !== 'All' && (
            <button
              onClick={() => setCategory('All')}
              className="flex items-center gap-1.5 text-xs bg-gray-900 text-white px-3 py-1.5 rounded-full font-medium hover:bg-gray-700 transition-colors">
          ...
          <div className="flex border rounded-xl overflow-hidden">
            <button onClick={() => setView('grid')}
              className={`px-4 py-2 text-sm font-medium transition-colors ${view === 'grid' ? 'bg-stay-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
              ☰ List
            </button>
            <button onClick={() => setView('map')}
              className={`px-4 py-2 text-sm font-medium transition-colors ${view === 'map' ? 'bg-stay-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
              🗺 Map
            </button>
          </div>
        </div>
          ...
          {!isLoading && listings.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500 text-lg">
                No {category === 'All' ? '' : category.toLowerCase() + ' '}stays found.
              </p>
