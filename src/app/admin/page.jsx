    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
        ...
        <p className="text-sm text-gray-500 text-center mb-6">
          Are you sure you want to delete <span className="font-semibold">"{listing.title}"</span>? This cannot be undone.
        </p>
      ...
      <div className="flex gap-4 mb-6 border-b">
        {['applications', 'listings', 'bookings'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`pb-2 text-sm font-medium capitalize border-b-2 -mb-px ${tab === t ? 'border-brand-500 text-brand-500' : 'border-transparent text-gray-400'}`}>
            {t}
          </button>
        ))}
      </div>
              ...
              <tbody>
                {apps.map(a => (
                  <tr key={a.id} className="border-b hover:bg-gray-50 cursor-pointer"
                    ...
                    <td className="py-3 pr-4">
                      {a.monthly_income ? `$${Number(a.monthly_income).toLocaleString()}/mo` : '—'}
                    </td>
                    <td className="py-3 pr-4">
                ...
                <tr key={l.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 flex gap-2">
                    <a href={`/admin/listings/${l.id}/edit`}
                      className="px-3 py-1 text-xs border rounded-lg hover:bg-gray-50">Edit</a>
                    <button onClick={() => setDeleteTarget(l)}
                      className="px-3 py-1 text-xs bg-red-100 text-red-600 rounded-lg">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
